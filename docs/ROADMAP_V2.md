...
| --- | --- | --- |
| P0 프리뷰 수집 | ✅ 배포 | — |
| **P0.5 라이브 갱신 이관** | ✅ 배포 | 계획에 없던 단계. §3의 P0.5 참고 |
| P1 선발 매치업 | ✅ 배포 | — |
| P2 라인업 탭 | ✅ 배포 | 예고 라인업만 표시. 예고↔실제 차이 뷰는 미구현 |
| P3 내 팀 · 등말소 | ✅ 배포 | — |
| P4 Who Are Ya 연계 | ✅ 배포 | 양쪽 다 배포. 정답 풀은 whoareya 주간 크론이 돌 때 Supabase에 반영된다 |
| **§6-1 URL 구조** | ✅ 배포 | 계획상 "지금 결정해야 할 유일한 장기 항목"이었다 |
| P5 알림 · iCal | ✅ 배포 | 실기기 알림 수신 확인은 사용자 몫 |
| P6 순위 · 매직넘버 | ✅ 배포 | 잔여 경기 상대 분포는 미착수 (§6-2) |

설계와 달라진 것:

- **P3의 전제가 틀렸다.** `Player/Register.aspx`가 `hfSearchDate`로 과거 날짜를 조회할 수 있어
  "하루 놓치면 복구 불가"가 아니다. Playwright도 필요 없고 순수 HTTP POST로 된다.
  그래서 diff 기반 이중 안전장치(`detected_by`)를 만들지 않았고, 대신 스케줄 작업이 3일치를 되메운다.
  대신 `roster_checks`를 두어 **조회했는데 0건인 날과 아예 조회하지 않은 날**을 구분한다.
- **P4에 Supabase 자격증명이 필요 없었다.** whoareya의 `players.json`이 공개돼 있어
  Pages Function이 서버사이드로 중계하면 된다. 프록시 한 개로 끝난다.
- **`standings.rank`는 신뢰할 수 있다.** 한화·롯데가 둘 다 8위로 내려온 것은 승률이 .440로 같아
  공동 순위를 준 것이지 오류가 아니었다(다른 날짜는 1~10위가 전부 구분됨). 앞서 적은 경고는 틀렸고,
  오늘 화면에 순위를 표시한다.
- **딥링크에서 상대 경로가 앱을 통째로 죽였다.** `./app.js`가 `/team/app.js`로 풀려 스크립트가
  로드되지 않았다. 자산 경로를 전부 루트 기준으로 바꿨다. 서비스 워커도 페이지 이동은 네트워크 우선으로
  바꿨다 — 캐시 우선이면 한 주소의 메타가 다른 주소에 그대로 굳는다.
- **웹 푸시는 페이로드를 싣지 않는다.** 암호화된 페이로드는 aes128gcm+ECDH를 직접 구현해야 하는데,
  빈 푸시는 VAPID JWT만 있으면 된다. 서비스 워커가 빈 푸시를 신호로 받아 `/api/push/alerts`에서
  내용을 가져간다. 실제 FCM이 가짜 토큰에 401이 아니라 **410**을 준 것으로 ES256 서명은 검증됐다.
  다만 **실기기에서 알림이 실제로 뜨는지는 확인하지 못했다** — 자동화 환경에서는 알림 권한이 떨어지지 않는다.
- **구독 주소는 실제 푸시 서비스 호스트만 받는다.** 서버가 그 주소로 POST를 보내므로
  아무 https URL이나 받으면 SSRF 중계기가 된다.
- **P2의 예고↔실제 차이 뷰는 넣지 않았다.** 종료 경기의 실제 출장은 이미 타자·투수 탭에 있고,
  차이만 따로 보여주는 화면은 값보다 소음이 클 것 같아 문구로 안내만 했다.
...
### P0.5 — 라이브 갱신을 GitHub 크론에서 떼어낸다 (완료, 2026-09-06)

P0을 배포하고 나서야 드러난 문제다. **수집 파이프라인은 멀쩡한데 실행되지 않고 있었다.**

`daily_update.yml`은 `'*/10 * * * *'`(10분)로 걸려 있었지만 실제 `schedule` 실행 간격은
**105~280분**이었고(2026-09-05~06 실측), 07-16~09-05 사이에는 51일간 한 번도 오지 않았다.
브라우저는 60초마다 폴링하는데 원본이 두 시간에 한 번 바뀌니 실시간일 수가 없었다.
GitHub의 `schedule` 이벤트는 SLA가 없고 고빈도 크론일수록 드랍된다 — 코드 문제가 아니다.

같은 뿌리에서 나온 부작용 두 가지도 함께 고쳤다.

- 화면의 `수집 지연` 경고가 임계 30분이라 **거의 상시 켜져 있었다**. 파이프라인이 둘로
  나뉘었으므로 라이브(5분)와 일정(6시간)을 각각의 임계로 판단한다.
- `game-polling`이 진행 중인 경기에 주는 `STARTED`가 라이브 상태 집합에 없어
  D1의 `status`가 `scheduled`에 머물렀다. 화면이 맞아 보인 것은 `inferLiveStatus`가
  응답 단계에서 시각으로 덮어썼기 때문이다. (crawling.py 쪽은 P0에서 이미 고쳐져 있었다.)

**구조**

```text
Cloudflare 크론 워커 (workers/live-cron, 1분)
  POST /api/live-refresh
    1. D1에서 오늘 경기 조회
    2. 대상이 없으면 외부 요청 0회로 종료
    3. 라이브 창(시작 20분 전~6시간 후) 경기 → game-polling → games UPDATE
    4. 라인업 미확정 경기(시작 6시간 전~시작, 4분 간격) → preview → game_previews UPSERT
```

- 로직은 Pages Function에 두고 워커는 크론 트리거만 담당한다. 워커에 D1 바인딩이 없다.
- 네이버 gameId는 우리 `game_id` + 시즌으로 만든다. 일정 API를 다시 부르지 않는다.
- 라이브 실행 로그는 KST 시간 단위 한 행으로 upsert한다(분당 1행이면 시즌에 수만 행).
- Actions의 `KBO_NAVER_LIVE`·`KBO_COLLECT_PREVIEWS`는 크론이 죽었을 때의 이중화로 남겼다.

**검증**: 배포 직후 지연 0.1분, 이후 1분 주기로 5회말→6회초 전환까지 반영 확인.
...
1. ~~하단 내비 4탭~~ → **4탭으로 결정, 배포됨** (오늘/일정/내 팀/설정).
2. ~~네이버 API 의존 범위~~ → **준비만 해두고 유지하기로.** 공식 GameCenter 폴백 경로를 §1 표에 기록했고
   (`GetLineUpAnalysis`, `LINEUP_CK`로 당일 발표 여부 구분 필요), `game_previews.source` 칼럼으로
   소스를 갈아끼울 수 있게 해뒀다. 실제 이전은 네이버가 막힐 때 한다.
3. ~~whoareya 데이터를 런타임에 읽을 것인가~~ → **런타임 프록시로 결정, 배포됨.** Supabase 자격증명 없이
   공개 `players.json`을 Pages Function이 중계한다.
4. **서비스 이름 `KBO GameDay` 확정** — 아직 미결. 유일하게 남은 사용자 결정 사항이다.
   딥링크·사이트맵·iCal 제목·푸시 문구가 전부 이 이름을 쓰고 있어 바꾸려면 지금이 가장 싸다.
