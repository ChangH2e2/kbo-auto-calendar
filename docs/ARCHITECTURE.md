...
| 데이터 | 실행 빈도 | 실행 주체 | 범위 |
| --- | --- | --- | --- |
| **경기 중 점수·이닝** | **1분** | **Cloudflare 크론 워커** | 오늘 경기 (시작 20분 전~6시간 후) |
| **경기 프리뷰·라인업** | **1분 검사, 경기당 4분 간격** | **Cloudflare 크론 워커** | 시작 6시간 전~시작, 라인업 확정 전까지 |
| 시즌 일정 | 30분 요청 (실제 2시간대) | GitHub Actions | 현재 시즌 전체 |
| 종료 경기 상세 | 6시간 | GitHub Actions | 최근 14일 |
| 예매 정책 | 운영자 변경 또는 주 1회 검증 | 운영자 | 구단별 |
| 공휴일 | 시즌 시작 전 및 월 1회 | GitHub Actions | 현재/다음 해 |

### GitHub Actions의 schedule에 라이브 데이터를 걸지 않는다

`schedule` 이벤트는 SLA가 없고 고빈도 크론일수록 대량으로 드랍된다. 2026-09-05~06에
10분 크론으로 걸어 둔 워크플로의 실제 실행 간격을 측정한 결과는 **105~280분**이었고,
07-16~09-05 사이에는 51일간 한 번도 오지 않았다. 코드 문제가 아니라 플랫폼 특성이다.

그래서 분 단위 신선도가 필요한 두 가지(경기 중 점수, 라인업 발표)는 **Cloudflare Cron Triggers**로 옮겼다.

```text
Cloudflare 크론 워커 (workers/live-cron, 1분)
  POST /api/live-refresh   Bearer LIVE_REFRESH_TOKEN
    1. D1에서 오늘 경기 조회
    2. 대상이 없으면 외부 요청 0회로 즉시 종료
    3. 네이버 game-polling  -> games UPDATE
    4. 네이버 preview       -> game_previews UPSERT
    5. ingestion_runs 에 job_type='live' 기록
```

- 로직은 전부 Pages Function에 있고 워커는 크론 트리거만 담당한다. 워커에 D1 바인딩을 두지 않는다.
- 네이버 gameId는 우리 `game_id`에 시즌을 붙여 만든다 (`20260906HHLT0` + `2026`). 일정 API를 한 번 더 부르지 않는다.
- 워커는 `workers_dev: false`라 공개 엔드포인트가 없다. 크론으로만 실행된다.
- `daily_update.yml`의 `KBO_NAVER_LIVE`·`KBO_COLLECT_PREVIEWS`는 크론이 멈췄을 때를 위한 이중화로 남겨 둔다.
  `game_previews`의 upsert 가드가 오래된 값의 덮어쓰기를 막으므로 두 경로가 충돌하지 않는다.
...
- 경기 전후 5시간 동안 브라우저가 60초마다 최신 경기 데이터를 재조회한다. 원본 수집은 크론 워커가 1분 주기로 담당한다.
- 진행 중인 경기가 응답에 있으면 `/api/games`의 캐시를 `s-maxage=10`으로 낮춘다. 기본값 60초는 분 단위 갱신을 그대로 지운다.
- 수집 실행이 30분 이상 없으면 성공 이력이어도 UI에서 `수집 지연`으로 표시한다.
- PWA 서비스 워커는 경기·예매 API를 최신 우선으로 캐시하고 네트워크 실패 시 마지막 정상 응답으로 fallback한다.
...
functions/api/live-refresh.js  라이브 점수·프리뷰 갱신 (크론 워커가 호출)
workers/live-cron/         Cloudflare 크론 트리거 워커
migrations/                D1 스키마
crawling.py                KBO 수집기
docs/                      제품·설계·운영 문서
