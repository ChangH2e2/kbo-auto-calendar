...
NC처럼 경기별 상태 확인에 회원 인증이 필요한 예매처는 `requires_login`으로 관리하며,
화면에서 로그인 필요 안내와 전용 버튼 문구를 표시한다.
롯데 공식 안내는 일반예매를 시리즈 2주 전 수·금 14시로 명시하고 비회원 예매를 허용하지
않으므로 해당 규칙을 `migrations/0006_lotte_ticket_policy.sql`로 반영한다.
