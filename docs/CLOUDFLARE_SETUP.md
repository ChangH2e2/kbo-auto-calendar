...
홈팀으로 기존 KBO 경기를 찾은 뒤 상태를 연결하며, 허용된 티켓링크 HTTPS 출처만 받는다.
최근 12시간 안에 확인한 경기별 값이 있으면 화면은 이를 일반 정책보다 우선하고 `예매 중` 또는
정확한 오픈 예정 시각으로 표시한다. 오래된 값은 자동으로 일반 예상값으로 강등한다.

티켓링크 공개 화면은 일반 브라우저에서는 정보를 제공하지만 WebDriver 자동 접근을 차단한다.
따라서 우회형 스크래퍼나 실패하는 스케줄 작업은 두지 않으며, 공식 공개 API 또는 허가된 연동이
확보되기 전까지 운영자가 확인한 값만 ingest한다.
