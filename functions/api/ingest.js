...
export async function onRequestPost(context) {
  ...
  const statements = rows.map((row) => context.env.KBO_DB.prepare(`INSERT INTO games
    ...
    ON CONFLICT(id) DO UPDATE SET season=excluded.season,starts_at=excluded.starts_at,date=excluded.date,time=excluded.time,away_team=excluded.away_team,home_team=excluded.home_team,venue=excluded.venue,status=excluded.status,away_score=excluded.away_score,home_score=excluded.home_score,status_note=excluded.status_note,away_line=excluded.away_line,home_line=excluded.home_line,away_rheb=excluded.away_rheb,home_rheb=excluded.home_rheb,hitter_details=excluded.hitter_details,pitcher_details=excluded.pitcher_details,holiday_name=excluded.holiday_name,source_updated_at=excluded.source_updated_at`).bind(row.id,row.season,row.starts_at,row.date,row.time,row.away_team,row.home_team,row.venue,row.status,row.away_score,row.home_score,row.status_note,row.away_line,row.home_line,row.away_rheb,row.home_rheb,row.hitter_details,row.pitcher_details,row.holiday_name,row.source_updated_at));
